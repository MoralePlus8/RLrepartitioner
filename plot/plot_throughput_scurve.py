#!/usr/bin/env python3
"""
绘制 2 核心配置下 Even / UCP / RL 三种策略相对于 LRU 基准的归一化吞吐量 S 曲线。
所有策略按 UCP 的排序顺序排列。
"""
...
STRATEGIES = {
    ...
    'RL':   BASE_DIR / 'rl_2core_modified',
}


def compute_ipc(csv_path: Path) -> float | None:
    """从 CSV 文件中读取最后两行，计算总体 IPC。"""
    try:
    ...
    if len(df) < 2:
        return None
    last_two = df.iloc[-2:]
    total_instructions = last_two['instructions'].sum()
    global_cycle = last_two['global_cycle'].iloc[0]
    if global_cycle == 0:
        return None
...
plt.tight_layout()
plt.savefig('plot/throughput_scurve_2core.png', dpi=300, facecolor='white')
plt.savefig('plot/throughput_scurve_2core.pdf', facecolor='white')
plt.close()
...
print(f"\n图像已保存为 plot/throughput_scurve_2core.png 和 .pdf")
...
plt.tight_layout()
plt.savefig('plot/throughput_stats_bar_2core.png', dpi=300, facecolor='white')
plt.savefig('plot/throughput_stats_bar_2core.pdf', facecolor='white')
plt.close()

print(f"柱状图已保存为 plot/throughput_stats_bar_2core.png 和 .pdf")
